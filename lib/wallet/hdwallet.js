...
let privacyConstanst = require('../constants');
...
class KeyWallet{
    constructor(){
       ...
       this.KeySet = new key.KeySet();
    }
    fromSpendingKey(spendingKey){
        this.Depth = 0;              // 1 byte
        this.ChildNumber = new Uint8Array(4);       // 4 bytes
        this.ChainCode = new Uint8Array(32);         // 32 bytes
        this.KeySet = new key.KeySet().fromSpendingKey(spendingKey);
        return this;
    }
...
function TestKeyWallet() {
    ...
    let keySerial = keyWallet.base58CheckSerialize(constants.PaymentAddressType);
