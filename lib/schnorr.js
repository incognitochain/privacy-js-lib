...
class SchnPrivKey { 
    ...
    constructor(sk, r){
        ...
        this.PK.G = pc.PedCom.G[constants.SK];
        this.PK.H = pc.PedCom.G[constants.RAND];
        this.PK.PK = this.PK.G.mul(this.SK).add(this.PK.H.mul(this.R))
    
    }
    GetPublicKey(GGenerator, HGenerator) {
        let G = ec.P256.decompress(GGenerator);
        let H = ec.P256.decompress(HGenerator);
        ...
        return PK.compress().concat(GGenerator).concat(HGenerator);
    }
    Sign(data){
        if ((this.R != 0) || (this.R != null)){
            let k1 = new common.BigInt(utils.RandInt());
            let k2 = new common.BigInt(utils.RandInt());
            let t = (this.PK.G.mul(k1.toRed(ec.moduleN).fromRed())).add(this.PK.H.mul(k2.toRed(ec.moduleN).fromRed()));
            // console.log(t.getX().toString(), " ", t.getY().toString());
            //Add Hash soon
            let E = Hash(t, data);
        
            // xe = Sk * e
            let xe = this.SK.toRed(ec.moduleN).redMul(E.toRed(ec.moduleN)).fromRed();
            let S1 = k1.toRed(ec.moduleN).redSub(xe.toRed(ec.moduleN));

            // re = Randomness * e
            let re = this.R.toRed(ec.moduleN).redMul(E.toRed(ec.moduleN)).fromRed();
            let S2 = k2.toRed(ec.moduleN).redSub(re.toRed(ec.moduleN));

            // let res = new Uint8Array(constants.BigIntSize*3);
            // res.set(E.toArray('be',constants.BigIntSize), 0);
            // res.set(S1.toArray('be',constants.BigIntSize), constants.BigIntSize);
            // res.set(S2.toArray('be',constants.BigIntSize), constants.BigIntSize*2);
            return E.toArray('be',constants.BigIntSize).concat(S1.toArray('be',constants.BigIntSize)).concat(S2.toArray('be',constants.BigIntSize));
        } else {
            let k = new common.BigInt(utils.RandInt());
            let t = (this.PK.G.mul(k.toRed(ec.moduleN).fromRed()));
            // console.log(t.getX().toString(), " ", t.getY().toString());
            //Add Hash soon
            let E = Hash(t, data);
            ...
            // xe = Sk * e
            let xe = this.SK.toRed(ec.moduleN).redMul(E.toRed(ec.moduleN)).fromRed();
            let S = k.toRed(ec.moduleN).redSub(xe.toRed(ec.moduleN));
            return E.toArray('be',constants.BigIntSize).concat(S.toArray('be',constants.BigIntSize));
        }
    }
}
class SchnPubKey {
    // constructor(publicKeyBytesArrays){
    //     this.PK = ec.P256.decompress(publicKeyBytesArrays.slice(0, constants.CompressPointSize));
    //     this.G = ec.P256.decompress(publicKeyBytesArrays.slice(constants.CompressPointSize, constants.CompressPointSize*2));
    //     this.H = ec.P256.decompress(publicKeyBytesArrays.slice(constants.CompressPointSize*2, constants.CompressPointSize*3));
    // }
    constructor(PK, G, H){
        this.PK = PK;
        this.G = G;
...
function Hash(point, bytes){
    ...
    // b = point.getX().toArray('be', constants.BigIntSize).concat(point.getY().toArray('be', constants.BigIntSize)).concat(bytes);
    ...
    return new common.BigInt(common.HashBytesToBytes(point.getX().toArray('be', constants.BigIntSize).concat(point.getY().toArray('be', constants.BigIntSize)).concat(bytes)),10,'be');
}
