...
class Coin {
    ...
    fromBytes(bytes) {
        ...
        offset += 1;
        if (lenField) {
            this.PublicKey = ec.Curve.curve.decompress(bytes.slice(offset, offset + lenField));
            offset += lenField;
        }
        ...
        offset += 1;
        if (lenField) {
            this.CoinCommitment = ec.Curve.curve.decompress(bytes.slice(offset, offset + lenField));
            offset += lenField;
        }
        ...
        offset += 1;
        if (lenField) {
            this.SerialNumber = ec.Curve.curve.decompress(bytes.slice(offset, offset + lenField));
            offset += lenField;
        }
...
function TestCoin() {
    ...
    coin.PublicKey = ec.Curve.curve.decompress(publicKey);
...
class OutputCoin {
    constructor() {
        this.CoinDetails = new Coin();
        this.CoinDetailsEncrypted = new CoinDetailEncrypted();
        return this;
    }
    ...
    toBytes() {
        ...
        if (!this.CoinDetailsEncrypted.isNull()){
            ...
            bytes.push(coinDetailsEncryptedBytes);
        } else{
            bytes.push(0x00);
        }
