...
class Coin {
    ...
    toBytes() {
        ...
        if (!this.PublicKey.eq(zeroPoint)) {
            ...
            partialBytes[0].set([constants.CompressPointSize], 0);
        ...
        if (!this.CoinCommitment.eq(zeroPoint)) {
            ...
            partialBytes[1].set([constants.CompressPointSize], 0);
        ...
        if (!this.SNDerivator.eq(0)) {
            partialBytes[2] = new Uint8Array(33);
            partialBytes[2].set([constants.BigIntSize], 0);
        ...
        if (!this.SerialNumber.eq(zeroPoint)) {
            ...
            partialBytes[3].set([constants.CompressPointSize], 0);
        ...
        // randomness
        if (!this.Randomness.eq(0)) {
            partialBytes[4] = new Uint8Array(33);
            partialBytes[4].set([constants.BigIntSize], 0);
    ...
    commitAll(){
        let values = [new BigInt(0), this.Value, this.SNDerivator, new BigInt(this.getPubKeyLastByte()), this.Randomness];
...
function TestCoin() {
    ...
    coin.Randomness = utils.RandScalar(constants.BigIntSize);
    coin.SNDerivator = utils.RandScalar(constants.BigIntSize);
