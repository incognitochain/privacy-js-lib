...
const ec = require("../ec");
const Pds = require("../pedersen").PedCom;
const utils = require('./utils');
const privacy_utils = require('../privacy_utils');
...
class SNPrivacyProof {
    ...
    verify(mess = null) {
        ...
        if (!Pds.commitAtIndex(this.zInput, this.zRInput, SND).eq(this.comInput.mul(x).add(this.tInput))) {
            return false;
        }
        if (!Pds.commitAtIndex(this.zSK, this.zRSK, SK).eq(this.comSK.mul(x).add(this.tSK))) {
            return false;
        }
        return this.serialNumber.mul(this.zSK.clone().add(this.zInput)).eq(Pds.G[SK].mul(x).add(this.tSN));
    }
}
...
class SNPrivacyWitness {
    ...
    prove(mess = null) {
        ...
        let tSK = Pds.commitAtIndex(eSK, dSK, SK);

        // calculate tSND = g_SND^eSND * h^dSND
        let tInput = Pds.commitAtIndex(eSND, dSND, SND);
