...
class ComZeroProof {
    constructor(commitmentValue, index, commitmentZeroS, z){
        this.commitmentValue = commitmentValue;
        this.index = index; 
        this.commitmentZeroS = commitmentZeroS;
        this.z = z;
    }
    toBytes(){
        return this.commitmentValue.compress().concat(this.index).concat(this.commitmentZeroS.compress()).concat(this.z);
    }
}

class ComZeroWitness {
    constructor(){
        this.commitmentValue = null;
        this.index = null;
        this.Rnd = null;
    }
    // static fromBytes(Bytes){
    //     let res = new comZeroWitness();
    //     res.commitmentValue = ec.P256.decompress(Bytes.slice(0,ec.CompressPointSize));
    //     res.index = Number(Bytes[ec.CompressPointSize]);
    //     res.Rnd = new common.BigInt(Bytes.slice(ec.CompressPointSize + 1, ec.CompressPointSize + 1 + ec.BigIntSize));
    // };

    set(commitmentValue, index, Rnd){
        // let res = new comZeroWitness();
        this.commitmentValue = commitmentValue;
        this.index = index;
        this.Rnd = Rnd;
    ...
    prove(){
        ...
        var sRnd = utils.RandInt(constant.BigIntSize).toRed(ec.moduleN).fromRed();
        ...
        return new ComZeroProof(this.commitmentValue, this.index, PedCom.CommitAtIndex(new common.BigInt(0), sRnd, this.index), sRnd.toArray('be',constant.BigIntSize))
...
module.exports ={ComZeroWitness, ComZeroProof};