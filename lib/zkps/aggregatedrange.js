...
const { BulletproofParams, generateChallengeForAggRange, EncodeVectors, AggParam } = require("./aggregaterangeparams");
...
class AggregatedRangeProof {
  ...
  verify() {
    ...
    let aggParam = new BulletproofParams()
    let extraNumber = numValuePad - AggParam.G.length / MAX_EXP
    if (extraNumber > 0) {
      aggParam = addBulletproofParams(extraNumber)
    } else {
      aggParam.G = AggParam.G.slice(0, numValuePad*MAX_EXP)
      aggParam.H = AggParam.H.slice(0, numValuePad*MAX_EXP)
