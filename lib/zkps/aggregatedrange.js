...
let p = new Promise(async resolve => {
  if (readFileSync) {
    ...
    let temp = await fetch('./privacy.wasm')
    let array = await temp.arrayBuffer();
    console.log("array: ", array);
    resolve(array);
  }

  // resolve(fs.readFile(join(__dirname, '../../privacy.wasm'), 'utf-8'))
}
...
class AggregatedRangeWitness {
  ...
  async prove() {
    ...
    console.log("wasm.aggregatedRangeProve", wasm.aggregatedRangeProve);
