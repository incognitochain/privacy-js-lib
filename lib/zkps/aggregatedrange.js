...
class AggregatedRangeProof {
  ...
  setBytes(bytes) {
    ...
    this.tauX = new BigInt(bytes.slice(offset, offset + BIG_INT_SIZE), 16, "be");
    offset = offset + BIG_INT_SIZE;

    this.tHat = new BigInt(bytes.slice(offset, offset + BIG_INT_SIZE), 16, "be");
    offset = offset + BIG_INT_SIZE;

    this.mu = new BigInt(bytes.slice(offset, offset + BIG_INT_SIZE), 16, "be");
    offset = offset + BIG_INT_SIZE;
