import {estimateMultiRangeProofSize} from "./aggregatedrange_utils";

let BigInt = require('bn.js');
let PedCom = require("../pedersen").PedCom;
let ec = require('../ec');
let utils = require('../constants_utils');
let P256 = ec.P256;
...
function estimateProofSize(nInput, nOutput, hasconstants){
    if (!hasconstants){
        ...
        return Math.ceil(sizeProof/1024.0)
    }
    let flagSize = 14+7*nInput+4*nOutput;
    let sizeOneOfManyProof = nInput * constants.ONE_OF_MANY_PROOF_SIZE;
    let sizeSNconstantsProof = nInput * constants.SN_PRIVACY_PROOF_SIZE;
    let sizeComOutputMultiRangeProof = int(estimateMultiRangeProofSize(nOutput));
  ...
  let sizeComInputShardID = constants.COMPRESS_POINT_SIZE;
    ...
    let sizeCommitmentIndices = nInput * constants.CMRingSize * constants.UINT64_SIZE;

    let sizeProof = sizeOneOfManyProof + sizeSNconstantsProof +
