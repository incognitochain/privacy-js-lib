...
class innerProductProof {
    ...
    setBytes(bytes) {
        ...
        for (let i = 0; i < lenLArray; i++) {
            this.l[i] = p256.decompress(bytes.slice(offset, offset + constant.CompressPointSize));
            offset = offset + constant.CompressPointSize;
        }
        this.r = [];
        for (let i = 0; i < lenLArray; i++) {
            this.r[i] = p256.decompress(bytes.slice(offset, offset + constant.CompressPointSize));
            offset = offset + constant.CompressPointSize;
        }
        this.a = new BigInt(bytes.slice(offset, offset + constant.BigIntSize), 16, "be");
        offset = offset + constant.BigIntSize;
        this.b = new BigInt(bytes.slice(offset, offset + constant.BigIntSize), 16, "be");
        offset = offset + constant.BigIntSize;
        this.p = p256.decompress(bytes.slice(offset, offset + constant.CompressPointSize));
    }
    verify(AggParameter) {
        let p = this.p;
