...
class innerProductProof {
    ...
    setBytes(bytes) {
        ...
        for (let i = 0; i < lenLArray; i++) {
            ...
            this.l[i].decompress(bytes.slice(offset, offset + constant.CompressPointSize));
            offset = offset + constant.CompressPointSize;
        }
        this.L = [];
        for (let i = 0; i < lenLArray; i++) {
            this.r[i] = p256.curve.point(0, 0);
            this.r[i].decompress(bytes.slice(offset, offset + constant.CompressPointSize));
            offset = offset + constant.CompressPointSize;
        }
        ...
        offset = offset + constant.BigIntSize;
        this.p = p256.curve.point(0, 0);
        this.p.decompress(bytes.slice(offset, offset + constant.CompressPointSize));
    }
    verify(AggParameter) {
        let p = this.p;
