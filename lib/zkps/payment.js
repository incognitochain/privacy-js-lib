...
class PaymentWitness {
    ...
    init(hasPrivacy, spendingKey, inputCoins, outputCoins, pkLastByteSender, commitments, commitmentIndices, myCommitmentIndices, fee) {
        ...
        this.comInputShardID = pc.PedCom.CommitAtIndex(new BigInt(pkLastByteSender), randInputShardID, constants.SHARD_ID);
        ...
        for (let i = 0; i < numInputCoin; i++) {
            ...
            this.oneOfManyWitness[i].set(commitmentTemps[i], commitmentIndices.slice(preIndex, preIndex + constants.CM_RING_SIZE), randInputIsZero[i], indexIsZero, constants.SK);
        ...
        this.aggregatedRangeWitness.set(outputValue, constants.MAX_EXP);
        // ---------------------------------------------------
...
class PaymentProof {
    ...
    toBytes() {
        ...
        for (let i = 0; i < this.commitmentIndices.length; i++) {
            partialBytes[13].set(this.commitmentIndices[i].toArray('be', constants.UINT64_SIZE), offset);
