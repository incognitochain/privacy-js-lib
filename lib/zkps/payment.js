...
class PaymentWitness {
    ...
    init(hasPrivacy, spendingKey, inputCoins, outputCoins, pkLastByteSender, commitments, commitmentIndices, myCommitmentIndices, fee) {
        ...
        this.comInputShardID = pc.PedCom.commitAtIndex(new BigInt(pkLastByteSender), randInputShardID, constants.SHARDID);
        ...
        for (let i = 0; i < numOutputCoin; i++) {
            ...
            cmOutputShardID[i] = pc.PedCom.commitAtIndex(new BigInt(shardID), randOutputShardID[i], constants.SHARDID);
...
class PaymentProof {
    ...
    toBytes() {
        ...
        let partialBytes = new Array(constants.NumProofProperties);
        ...
        for (let i = 0; i < this.commitmentIndices.length; i++) {
            partialBytes[13].set(this.commitmentIndices[i].toArray('be', constants.Uint64Size), offset);
            offset += constants.Uint64Size;
        }
        paymentProofSize += partialBytes[13].length;

        let bytes = new Uint8Array(paymentProofSize);
        let index = 0;
        for (let i = 0; i < constants.NumProofProperties; i++) {
