...
class OneOutOfManyWitness {
  ...
  async prove() {
    ...
    for (let j = 0; j < n; j++) {
      ...
      // cl = Com(l, r)
      cl[j] = pc.PedCom.commitAtIndex(indexInt, r[j], SK);

      // ca = Com(a, s)
      ca[j] = pc.PedCom.commitAtIndex(a[j], s[j], SK);

      // cb = Com(la, t)
      let la = indexInt.mul(a[j]);
      cb[j] = pc.PedCom.commitAtIndex(la, t[j], SK);
    ...
    for (let k = 0; k < n; k++) {
      ...
      cd[k] = pc.PedCom.commitAtIndex(new bn(0), u[k], SK);
    ...
    for (let j = 0; j < n; j++) {
      x = zkpUtils.generateChallenge([x.toArray('be', BIG_INT_SIZE), cl[j].compress(), ca[j].compress(), cb[j].compress(), cd[j].compress()]);
    }
...
class OneOutOfManyProof {
  ...
  verify() {
    ...
    let x = new bn(0);
    for (let j = 0; j < n; j++) {
      x = zkpUtils.generateChallenge([utils.addPaddingBigInt(x, BIG_INT_SIZE), this.cl[j].compress(), this.ca[j].compress(), this.cb[j].compress(), this.cd[j].compress()])
    }

    for (let i = 0; i < n; i++) {
      //Check cl^x * ca = Com(f, za)
      let leftPoint1 = this.cl[i].mul(x).add(this.ca[i]);
      let rightPoint1 = pc.PedCom.commitAtIndex(this.f[i], this.za[i], SK);
      ...
      let rightPoint2 = pc.PedCom.commitAtIndex(new bn(0), this.zb[i], SK);
    ...
    let rightPoint3 = pc.PedCom.commitAtIndex(new bn(0), this.zd, SK);
