...
class OneOutOfManyWitness {
    ...
    prove() {
        ...
        for (let k = 0; k < n; k++) {
            ...
            cd[k] = pc.PedCom.CommitAtIndex(new common.BigInt(0), u[k], constants.SK);
        ...
        // let right1 = pc.PedCom.CommitAtIndex(new common.BigInt(0), za[0], constants.SK);
        //
        // if (left1.eq(right1)){
        ...
        for (let i=0; i<N; i++){
            let iBinary = utils.ConvertIntToBinary(i, n);
            let exp = new common.BigInt(1);
            let fji = new common.BigInt(1);
        ...
        for (let k =0; k<n; k++){
            let xk = x.pow(new common.BigInt(k));
            xk = xk.umod(P256.n);
        ...
        let rightPoint = pc.PedCom.CommitAtIndex(new common.BigInt(0), zd, constants.SK);
...
function TestGetCoefficient(){
    ...
    a[0] = new common.BigInt([28, 30, 162, 177, 161, 127, 119, 10, 195, 106, 31, 125, 252, 56, 111, 229, 236, 245, 202, 172, 27, 54, 110, 9, 9, 8, 56, 189, 248, 100, 190, 129]);
    a[1] = new common.BigInt([144, 245, 78, 232, 93, 155, 71, 49, 175, 154, 78, 81, 146, 120, 171, 74, 88, 99, 196, 61, 124, 156, 35, 55, 39, 22, 189, 111, 108, 236, 3, 131]);
    a[2] = new common.BigInt([224, 15, 114, 83, 56, 148, 202, 7, 187, 99, 242, 4, 2, 168, 169, 168, 44, 174, 215, 111, 119, 162, 172, 44, 225, 97, 236, 240, 242, 233, 148, 49]);

    let res = GetCoefficient([0,1,1], 3, 3, a, [0,1,1]);

    // let expectedRes = new common.BigInt(-6);
