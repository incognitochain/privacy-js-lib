...
class OneOutOfManyWitness {
  ...
  async prove() {
    ...
    if (typeof(oneOutOfManyProve) != "undefined"){
...
class OneOutOfManyProof {
  ...
  toBytes() {
    ...
    for (let i = 0; i < n; i++) {
      console.log("this.ca[i] ", this.ca[i]);
      let tmp = this.ca[i].compress();
      console.log("this.ca[i] bytes : ", tmp);
  ...
  setBytes(bytes) {
    ...
    this.cl = new Array(n);
    for (let i = 0; i < n; i++) {
      let tmp = bytes.slice(offset, offset + constants.COMPRESS_POINT_SIZE);
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.ca[i] = P256.decompress(bytes.slice(offset, offset + constants.COMPRESS_POINT_SIZE));
      offset = offset + constants.COMPRESS_POINT_SIZE;
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.cb[i] = P256.decompress(bytes.slice(offset, offset + constants.COMPRESS_POINT_SIZE));
      offset = offset + constants.COMPRESS_POINT_SIZE;
    }
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.cd[i] = P256.decompress(bytes.slice(offset, offset + constants.COMPRESS_POINT_SIZE));
      offset = offset + constants.COMPRESS_POINT_SIZE;
    }
    ...
    this.f = new Array(n);
    for (let i = 0; i < n; i++) {
      this.f[i] = new bn(bytes.slice(offset, offset + constants.BIG_INT_SIZE));
    ...
    this.za = new Array(n);
    for (let i = 0; i < n; i++) {
      this.za[i] = new bn(bytes.slice(offset, offset + constants.BIG_INT_SIZE));
      offset = offset + constants.BIG_INT_SIZE;
    }
    ...
    this.zb = new Array(n);
    for (let i = 0; i < n; i++) {
      this.zb[i] = new bn(bytes.slice(offset, offset + constants.BIG_INT_SIZE));
      offset = offset + constants.BIG_INT_SIZE;
    }
    console.log("this.zb, ", this.zb);

    this.zd = new bn(bytes.slice(offset, offset + constants.BIG_INT_SIZE));
