...
let keyWallet = require('../wallet/hdwallet');
let constantsWallet = require('./../wallet/constants');
...
async function prepareForTx(spendingKeyStr){
    ...
    let response = await GetOutputCoin(paymentAddrSerialize, readOnlyKeySerialize);
    let allOutputCoins;
    if (response.status === 200){
        allOutputCoins = response.data.Result.Outputs[readOnlyKeySerialize];
    }

    // check whether each output coin is spent or not
...
class Tx {
    ...
    init(senderSK, paymentInfo, inputCoins, fee, hasPrivacy, db, tokenID, metaData) {
        ...
        let sumOutputValue = new BigInt(0);
        ...
        let sumInputValue = new BigInt(0);
        ...
        witness.init(hasPrivacy, new BigInt(senderSK, 'be', constants.BigIntSize), inputCoins, outputCoins, pkLastByteSender, commitmentProving, commitmentIndices, myCommitmentIndices, fee);
        ...
        if (hasPrivacy) {
            ...
            this.sigPrivKey.set(senderSK, 0);
            this.sigPrivKey.set(new BigInt(0).toArray(), senderSK.length);
        }
    ...
    sign() {
        ...
        let sk = new BigInt(this.sigPrivKey.slice(0, constants.BigIntSize));
        let r = new BigInt(this.sigPrivKey.slice(constants.BigIntSize));
