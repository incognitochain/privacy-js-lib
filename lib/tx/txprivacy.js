...
function TestTx(){
    ...
    let paymentInfo = [];
    paymentInfo[0] = new key.PaymentInfo(new key.PaymentAddress().fromSpendingKey(skReceiver[0]), 5);
    paymentInfo[1] = new key.PaymentInfo(new key.PaymentAddress().fromSpendingKey(skReceiver[1]), 4);
    ...
    inputCoins[0] = new coin.InputCoin().set(key.GeneratePublicKey(skReceiver[0]),
        snderivators[0], vrf.Eval(skReceiver[0], snderivators[0], pc.PedCom.G[constants.SK]),
        utils.RandInt(), 10, null);

    inputCoins[1] = new coin.InputCoin().set(key.GeneratePublicKey(skReceiver[1]),
        snderivators[1], vrf.Eval(skReceiver[1], snderivators[1], pc.PedCom.G[constants.SK]),
        utils.RandInt(), 10, null);


    tx.init(skSender, paymentInfo, inputCoins, 0, false, null, null, null);
    console.log(tx);
}
