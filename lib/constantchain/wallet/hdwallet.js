...
class KeyWallet {
  ...
  getIntermediary(childIdx) {
    ...
    let hmac = CryptoJS.HmacSHA512(sha512(""), this.ChainCode);
    hmac.write(data);
    let intermediary = hmac.sum(null)
    return intermediary;
  }
...
function NewMasterKey(seed) {
  let hmac = hmac.init(sha512(""), "Constant Seed");
  hmac.write(seed);
  let intermediary = hmac.sum(null)

  // Split it into our PubKey and chain code
  let keyBytes = intermediary.slice(0, 33)  // use to create master private/public keypair
  let chainCode = intermediary[32] // be used with public PubKey (in keypair) for new Child keys
  let keySet = new keyset.KeySet();
  keySet.generateKey(seed);

  let keyWallet = new KeyWallet();
  keyWallet.KeySet = keySet;
  keyWallet.ChainCode = chainCode;
  keyWallet.Depth = 0;
  keyWallet.ChildNumber = [0, 0, 0, 0];
  return keyWallet;
}
