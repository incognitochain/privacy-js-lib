...
class Tx {
    ...
    init(senderSK, paymentInfo, inputCoins, fee, hasPrivacy, paymentAddrSerialize, inputCoinStrs, db, tokenID, metaData) {
        ...
        if (hasPrivacy) {
            let randCommitments = randomCommitmentsProcess(paymentAddrSerialize, inputCoinStrs);
            ...
            commitments = randCommitments.commitments;
            ...
            commitmentProving = new Array(inputCoins.length * constants.CM_RING_SIZE);
        ...
        while (ok) {
            let sndOut = new bn(0);
            for (i = 0; i < paymentInfo.length; i++) {
                sndOut = utils.RandInt(constants.BIG_INT_SIZE);
                while (true) {
                    //todo:
...
async function TestTx() {
    ...
    let tx = new Tx();
    console.log("---------- BEFORE CREATE TX res input coin : ", res.inputCoins.resultOutputCoins);
    tx.init(res.senderKeySet.PrivateKey, paymentInfos, res.inputCoins.resultOutputCoins, new bn.BN(0), true, null, null, null);
    console.log("***************Tx: ", tx)
    // console.log("res: ", res);
