...
class Tx {
    ...
    async init(senderSK, paymentAddrSerialize,  paymentInfo, inputCoins, inputCoinStrs, fee, hasPrivacy, db, tokenID, metaData) {
        ...
        let transmissionKeyPoint = P256.decompress(senderKeySet.PaymentAddress.TransmisionKey);
        ...
        if (hasPrivacy) {
            ...
            for (i = 0; i < this.Proof.outputCoins.length; i++) {
                let publicKeyPoint = P256.decompress(paymentInfo[i].PaymentAddress.PublicKey);
                this.Proof.outputCoins[i].encrypt(publicKeyPoint);
