...
async function GetOutputCoin(paymentAdrr, viewingKey) {
    ...
    const result = await httpRequest.requestAPI(data);
    ...
    if (result.status !== 200) {
        ...
        return {
            outCoins: result.data.Result.OutCoins[viewingKey],
            err: null
        }
    }
}
...
async function CheckExistSerialNumber(paymentAddr, serialNumberStrs) {
    return await axios({
        method: 'get',
...
async function prepareForTx(spendingKeyStr) {
    ...
    if (res.err === null){
        ...
        console.log('ERR when call API: ', res.err);
    }
    ...
    for (let i = 0; i < allOutputCoinStrs.length; i++) {
        allOutputCoinStrs[i].SerialNumber = P256.g.derive(new bn.BN(myKeyWallet.KeySet.PrivateKey), new bn.BN(allOutputCoinStrs[i].SNDerivator))
    }

    console.log("all output coin after calculating serial number: ", allOutputCoinStrs);
