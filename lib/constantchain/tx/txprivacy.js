...
async function getUnspentCoin(inputCoins, paymentAddrSerialize){
    ...
    let serialNuberStrs = new Array(inputCoins.length);

    for (let i =0; i<inputCoins.length; i++){
        // inputCoins[i].CoinDetails.SerialNumber = P256.g.derive(new bn.BN(myKeyWallet.KeySet.PrivateKey), new bn.BN(inputCoins[i].CoinDetails.SNDerivator));
    ...
    let existed = [];
    if (resSN.err !== null){
        console.log('ERR when call API has serial number: ', resSN.err);
    }else{
...
async function prepareInputForTx(spendingKeyStr, paymentInfos) {
    ...
    // get unspent coin
    let unspentCoins = getUnspentCoin(inputCoins, paymentAddrSerialize);
    ...
    return ChooseBestCoinToSpent(unspentCoins, amount);
}
