...
Base.base.BasePoint.prototype.derive = function (seed, derivator) {
    ...
    console.time("1.2");
    let seedTemp = bigInt(seed.toString());
    let derivatorTemp = bigInt(derivator.toString());
    let temp = seedTemp.plus(derivatorTemp);
    temp = temp.modInv(n);
    let result = (this.isSafe() ? this.mul(new BN(temp.toString())) : null);
    console.timeEnd("1.2")
