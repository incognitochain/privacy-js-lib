...
Base.base.BasePoint.prototype.derive = function (seed, derivator) {
    ...
    let seedTemp = bigInt(seed.toNumber())
    let derivatorTemp = bigInt(derivator.toNumber())
    let temp = seedTemp.plus(derivatorTemp)
    temp = temp.modInv(bigInt(P256.n.toNumber()))
    return (this.isSafe() ? this.mul(new BN(temp.toString())) : null)
};
