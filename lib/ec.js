...
function modInverse(a, m) {
    a = a % m;
    for (var x = BigInt(1); x < m; x = x + BigInt(1)) {
        if ((a * x) % m == 1)
            return x;
    }
}
...
Base.base.BasePoint.prototype.derive = function (seedInRed, derivator) {
    console.time("1.1");
    try {
        this.mul((seedInRed.redAdd(derivator.toRed(N))).redInvm().fromRed())
    } catch (e) {
        console.log("ERR1", e);
    }
    console.timeEnd("1.1");
    ...
    console.time("1.2");
    let temp = (BigInt(seedInRed.fromRed().toString()) + BigInt(derivator.toString()));
