...
Base.base.BasePoint.prototype.derive = function (seed, derivator) {
    console.time("1.1");
    // (this.isSafe() ? this.mul((seed.toRed(N).redAdd(derivator.toRed(N))).redInvm().fromRed()) : null);
    try {
        this.mul((seed.toRed(N).redAdd(derivator.toRed(N))).redInvm().fromRed())
    } catch (e) {
    }
    console.timeEnd("1.1");

    console.time("1.2");
    const seedTemp = bigInt(seed.toString());
    const derivatorTemp = bigInt(derivator.toString());
    const temp = seedTemp.plus(derivatorTemp);
    const temp1 = temp.modInv(n);
    let result;
    // result = (this.isSafe() ? this.mul(new BN(temp1.toString())) : null);
    try {
        result = this.mul(new BN(temp1.toString()));
    } catch (e) {
        result = null;
    }
    console.timeEnd("1.2")
