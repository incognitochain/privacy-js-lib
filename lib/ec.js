...
const moduleP = BigInt.red(P256.curve.p.clone());
const moduleN = BigInt.red(P256.n.clone());
...
P256.isOnCurve = function (point) {
    let x = point.getX();
    let y = point.getY();
    return (y.toRed(moduleP).redMul(y.toRed(moduleP)).fromRed()).cmp(x.toRed(moduleP).redPow(new BigInt(3)).redSub(x.toRed(moduleP).redMul((new BigInt(3)).toRed(moduleP))).redAdd(P256.B.toRed(moduleP)).fromRed()) === 0;
}
...
P256.randomize = function () {
    ...
    while (1) {
        ...
        try {
            res = P256.curve.pointFromX(x, utils.RandBytes(1)[0] % 2);
        } catch (error) {
            res = null;
        }
        if ((res != null) && (res.isSafe())) {
