...
Base.base.BasePoint.prototype.derive = function (seedInRed, derivator) {
    let result;
    console.log("seedInRed", seedInRed)
    // (this.isSafe() ? this.mul((seed.toRed(N).redAdd(derivator.toRed(N))).redInvm().fromRed()) : null);
    try {
        result = this.mul((seedInRed.redAdd(derivator.toRed(N))).redInvm().fromRed())
    } catch (e) {
    }
