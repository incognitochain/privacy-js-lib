...
ECC.curve.short.prototype.pointFromX = function pointFromX(x, odd) {
    x = new BigInt(x, 16);
    if (!x.red)
      x = x.toRed(this.red);
...
Base.base.BasePoint.prototype.marshalJSON = function () {
    let stringJSON = JSON.stringify(base58.checkEncode(this.compress(), cs.PRIVACY_VERSION));
...
P256.unmarshalJSON = function (data) {
    ...
    if (res.version != cs.PRIVACY_VERSION) {
        throw new Error("Decode failed! Wrong Privacy version!");
    }
...
P256.randomize = function () {
    let res = null;
    while (1) {
        x = new BigInt(utils.randBytes(cs.BIG_INT_SIZE));
        try {
