...
function randBytes(n = constants.BIG_INT_SIZE) {
  try {
    ...
    console.log(e);
    let randomFunc = Utility.getRandomBytesFunc();
    if (randomFunc) {
      return randomFunc(n);
    } else{
...
function Utility() {
  let RandomBytesFunc = null;
  return {
    set: function setRandomBytesFunc(f) {
      RandomBytesFunc = f;
    },
    get: function getRandomBytesFunc() {
        return RandomBytesFunc;
    }
  }
}

// Utility.RandomBytesFunc = null;
// Utility.A = "A";
