...
//         while (new common.BigInt(spendingKey, 10, 'be').gt(ec.P256.n)) {
...
//         var pk = ec.P256.g.mul(new common.BigInt(spendingKey, 10, 'be'));
//
...
function GeneratePublicKey(spendingKey) {
    var publicKey = ec.P256.g.mul(new common.BigInt(spendingKey, 10, 'be'));
    console.log('Public key is safe: ', publicKey.issafe());
...
//         var tk = ec.P256.g.mul(new common.BigInt(receivingKey, 10, 'be'));
//
...
function GenerateTranmissionKey(receivingKey) {
    let transKey = ec.P256.g.mul(new common.BigInt(receivingKey, 10, 'be'))
    console.log('Public key is safe: ', transKey.issafe());
    return transKey.compress();
}
...
module.exports = {GenerateSpendingKey, GeneratePublicKey, GenerateTranmissionKey, GenerateReceivingKey, PaymentAddress, ViewingKey, PaymentInfo, KeySet}
