...
function GeneratePublicKey(spendingKey) {
    return (ec.P256.g.mul(new common.BigInt(spendingKey, 10, 'be'))).compress() ;
}
...
function GenerateTranmissionKey(receivingKey) {
    return (ec.P256.g.mul(new common.BigInt(receivingKey, 10, 'be'))).compress();
}
...
class KeySet {
    constructor(spendingKey){
        ...
        this.PaymentAddress = new PaymentAddress(spendingKey);
